<?php
  ...
  if($_GET['id']) {
    $id = (int)$_GET['id'];
    $articles_array = delete_article($id);
  }
?>
...
<body>
  ...
  <div class="container">
    <div class="block-setting">
      <h2 class="cyber-h2 text-center">Управление записями</h2>
      <?foreach($articles_array as $key => $value) {?>
      <h2 class="cyber-h5"><?php echo $value['title'] ?></h2>
      <h6 class="cyber-h6">Дата: <?php echo $value['date'] ?></h6>
      <div class="row">
        <div class="col-10">
          ...
          <a href="setting.php?id=<?php echo $value['id'] ?>" class="btn btn-cyber" type="submit" name="action"
            value="Update">Удалить</a>
          <p class="text-left cyber-ps">Опубликовано пользователем: <?php echo $value['creator'] ?></p>
        </div>
        <div class="col-2">
          <img src="<?php echo $value['image'] ?>" class="img-fluid img-shadow img_setting">
        </div>
      </div>
